<template>
  <div class="btc-member-center" @click="ChoiceAddress(false)">
    ...
    <div class="btc-container-block">
      ...
      <div class="btc-b" v-if="route === 'withdraw'">
        <div class="btc-withdraw-form">
          ...
          <div @click.stop="ChoiceAddress(!choice)" class="btc-withdraw-address btc-b-l" :style="{background:`url('${requireImg('select')}') 100% 100%`}">
            {{ $t('withdraw_currency.withdraw_currency_address') }}
          </div>
          <div v-if="choice" @click.stop="ChoiceAddress(true)" class='btc-b btc-address-contain '>
          ...
          <template v-else>
            <div class="btc-marginT15 btc-withdraw-add" style="display: flex" v-if="withdrawAddress">
              <basic-input class="btc-withdraw-all"  :placeholder='$t("withdraw_currency.remark_label")'>
              </basic-input>
              <basic-input class="btc-withdraw-all"  :placeholder='$t("withdraw_currency.withdraw_currency_address")'>
              </basic-input>
            </div>
    ...
    <basic-table :table='withdrawRecord'>
...
export default {
  ...
  computed: {
    withdrawRecord () {
      return {
        captionTitle: this.$t('withdraw_currency.btc_withdraw_currency_record'),
        Item: [{
          content: [
