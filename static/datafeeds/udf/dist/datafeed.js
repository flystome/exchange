...
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, updateFrequency,market) {
  ...
  this._lastTime=0;
  this._lastBarValue={};
  this.kindledata = null;
  this.oldResolution = 0;
  this.serverTime = 0;
  this.prev_json = null;
  this.first_req = true;
  this.totalTrade = 0;
...
Datafeeds.UDFCompatibleDatafeed.prototype.defaultConfiguration = function() {
  return {
    ...
    supported_resolutions: ['1', '5', '15', '30', '60', '120', '240', '360', '720', '1440', '10080'],
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
  ...
  if (that.first_req) {
    xhr = this._send(this._datafeedURL + this._historyURL, {
      symbol: symbolInfo.ticker.toUpperCase(),
      resolution: resolution,
    })
    this.oldResolution = resolution;
    that.first_req = false;
  } else if ( that.prev_json) {
    xhr = this._send(this.prev_json);
  } else {
    return ;
  }
  ...
  xhr.done(function(response) {
    ...
    var barLast = bars[bars.length-1];

    if(!that.prev_json && !nodata){
      that._lastTime = data.t[data.t.length-1]
      that._lastBarValue = {
        time: barLast.at * 1000,
        close: +barLast.close,
        open: +barLast.close,
        high: +barLast.close,
        low: +barLast.close,
        volume: +barLast.volume
      };
    }

    that._send("/api/v2/trading_views/time").done(function(res){
      that.serverTime = res*1000;
    })
    that.prev_json = data.prev_json
...
Datafeeds.UDFCompatibleDatafeed.prototype.getKindle = function(){
  var that = this;
  document.addEventListener("kindleData", function(e){
    that.totalTrade = +that.totalTrade + (+e.data.trades[0].amount);
    that.kindledata = e.data.trades[0];
    that.kindledata.total = that.totalTrade;
  })
};

Datafeeds.UDFCompatibleDatafeed.prototype.getBars1 = function(symbolInfo, resolution, rangeStartDate,rangeEndDate, onDataCallback, onErrorCallback) {
   // timestamp sample: 1399939200
  var that = this;
  if (rangeStartDate > 0 && (rangeStartDate + '').length > 10) {
    throw new Error(['Got a JS time instead of Unix one.', rangeStartDate, rangeEndDate]);
  ...
  if(that.kindledata && JSON.stringify(that.kindledata) != '{}'){
    var data = parseJSONorNot(that.kindledata);
    var bars = [];
    ...
    // if(data.price<that._lastBarValue.low){
    //   barValue.low = data.price
    // }
    // if(data.price>that._lastBarValue.high){
    //   barValue.high = data.price
    // }
    that._lastBarValue.close = data.price;
    // that._lastBarValue.open = +that._lastBarValue.open || +data.price
    // if(data.price<that._lastBarValue.low){
    //   that._lastBarValue.low = data.price
    // }
    // if(data.price>that._lastBarValue.high){
    //   that._lastBarValue.high = data.price
    // }
    ...
    bars.push(barValue);
    onDataCallback(bars, { noData: nodata});
    that.kindledata = null;
  }else{
    var bars = [];
    if(isNaN(that._lastBarValue.time)){
      that._lastBarValue.time = that._lastTime;
    }
    bars.push(that._lastBarValue);
    onDataCallback(bars, { noData: nodata});
  }

  that.serverTime= that.serverTime + 10 * 1000;
  ...
  try {
    for (var symbolIndex = 0; symbolIndex < data.symbol.length; ++symbolIndex) {
      ...
      var symbolInfo = {
        ...
        intraday_multipliers: tableField(data, 'intraday-multipliers', symbolIndex) || ['1', '5', '15', '30', '60', '120', '240', '360', '720', '1440', '10080'],
...
Datafeeds.DataPulseUpdater = function(datafeed, updateFrequency) {
  ...
  var updates = function() {
    ...
    for (var listenerGUID in that._subscribers) {
      ...
      (function(_subscriptionRecord) { // eslint-disable-line
        that._datafeed.getBars1(_subscriptionRecord.symbolInfo, resolution, datesRangeLeft,datesRangeRight, function(bars) {
          that._requestsPending--;
          ...
          if (!bars[0] || bars.length === 0) {
            return;
          }

          var lastBar = bars[bars.length - 1];
          if((''+lastBar.time).length == 13){
            lastBar.time = lastBar.time/1000
          }

          var lastBarTime = lastBar.time;
          var lastserverTime = that._datafeed.serverTime/1000;
          var lastTime = that._datafeed._lastTime;
          var resolutionTime = that.periodLengthSeconds(resolution, 1);
          var subscribers = _subscriptionRecord.listeners;

          if(!isNaN(lastserverTime)&&lastBarTime>=lastTime){
            if(lastserverTime < lastTime){
              return ;
            }else if(lastserverTime < (lastTime + resolutionTime)){
              lastBar.time = that._datafeed._lastTime*1000;
              lastBar.volume = that._datafeed.totalTrade;
              lastBar.open = that._datafeed._lastBarValue.open || lastBar.open;
              if(lastBar.low<that._datafeed._lastBarValue.low){
                that._datafeed._lastBarValue.low = lastBar.low
              }else {
                lastBar.low = that._datafeed._lastBarValue.low;
              }
              if(lastBar.high>that._datafeed._lastBarValue.high){
                that._datafeed._lastBarValue.high = lastBar.high
              }else {
                lastBar.high = that._datafeed._lastBarValue.high;
              }
              for (var i = 0; i < subscribers.length; ++i) {
                subscribers[i](lastBar);
              }
            }else if(lastserverTime >= (lastTime + resolutionTime)){
              lastBar.time = (lastTime + resolutionTime)*1000;
              that._datafeed._lastTime = that._datafeed._lastTime + resolutionTime;
              that._datafeed.totalTrade = 0
              that._datafeed._lastBarValue = {//初始化
                time: (lastTime + resolutionTime)*1000,
                open: lastBar.close,
                high: lastBar.close,
                low: lastBar.close,
                close: lastBar.close,
  ...
  if (typeof updateFrequency != 'undefined' && updateFrequency > 0) {
    clearInterval(timer);
    this._datafeed.getKindle();
    var timer = setInterval(updates, updateFrequency);
  }
};
